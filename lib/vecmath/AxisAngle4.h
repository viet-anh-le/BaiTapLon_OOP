...
template<class T>
void AxisAngle4<T>::setFromMat(T m00, T m01, T m02, T m10, T m11, T m12, T m20,
                               T m21, T m22) {
...
void AxisAngle4<T>::setFromQuat(T x, T y, T z, T w) {
    ...
    this->angle = 2.0 * VmUtil<T>::atan2(
            sin_a2, w); // 0 <= angle <= PI , because 0 < sin_a2
    this->x = x;
    this->y = y;
    this->z = z;
}
...
template<class T>
VM_STRING_STD::string AxisAngle4<T>::toString() const { VM_TOSTRING }
...
template<class T>
VM_IOSTREAM_STD::ostream &
operator<<(VM_IOSTREAM_STD::ostream &o,
           const VM_VECMATH_NS::AxisAngle4<T> &a1) {
    return o << "(" << a1.x << "," << a1.y << "," << a1.z << "," << a1.angle
             << ")";
}

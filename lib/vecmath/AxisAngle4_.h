...
public:
     ...
     */
    AxisAngle4(const T a[]) : x(a[0]), y(a[1]), z(a[2]), angle(a[3]) {}
    ...
    // helper method
protected:
    void setFromMat(T m00, T m01, T m02, T m10, T m11, T m12, T m20, T m21,
                    T m22);
...
public:
    /**
     * Returns true if all of the data members of AxisAngle4 t1 are equal to the
     ...
     * (ie, returns true for equals(AxisAngle4) ) will return the same hash
     * number. Two vectors with different data members may return the same hash
     * value, although this is not likely.
     */
    size_t hashCode() const { return VmUtil<T>::hashCode(sizeof *this, this); }

    /**
     * Returns a string that contains the values of this AxisAngle4. The form is
     * (x,y,z,angle).
    ...
    bool operator==(const AxisAngle4 &m1) const { return equals(m1); }
};

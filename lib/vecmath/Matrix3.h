...
template<class T>
Matrix3<T>::Matrix3(T n00, T n01, T n02, T n10, T n11, T n12, T n20, T n21,
                    T n22)
        : m00(n00), m01(n01), m02(n02), m10(n10), m11(n11), m12(n12), m20(n20),
          m21(n21), m22(n22) {}

template<class T>
Matrix3<T>::Matrix3(const T v[])
        : m00(v[0]), m01(v[1]), m02(v[2]), m10(v[3]), m11(v[4]), m12(v[5]),
          m20(v[6]), m21(v[7]), m22(v[8]) {}

#ifdef VM_INCLUDE_CONVERSION_FROM_2DARRAY
template <class T>
Matrix3<T>::Matrix3(const T v[][3])
    : m00(v[0][0]), m01(v[0][1]), m02(v[0][2]), m10(v[1][0]), m11(v[1][1]),
      m12(v[1][2]), m20(v[2][0]), m21(v[2][1]), m22(v[2][2]) {}
#endif
...
template<class T>
void Matrix3<T>::set(T m00, T m01, T m02, T m10, T m11, T m12, T m20, T m21,
                     T m22) {
    this->m00 = m00;
    this->m01 = m01;
    this->m02 = m02;
...
#ifdef VM_INCLUDE_CONVERSION_FROM_2DARRAY
template <class T> void Matrix3<T>::set(const T m[][3]) {
  assert(m != 0);
  m00 = m[0][0];
  m01 = m[0][1];
  m02 = m[0][2];
...
template<class T>
T Matrix3<T>::getScale() const { return SVD(0); }
...
template<class T>
void Matrix3<T>::sub(T scalar) { add(-scalar); }
...
void Matrix3<T>::add(const Matrix3 &m1, const Matrix3 &m2) {
    ...
    set(m1.m00 + m2.m00, m1.m01 + m2.m01, m1.m02 + m2.m02, m1.m10 + m2.m10,
        m1.m11 + m2.m11, m1.m12 + m2.m12, m1.m20 + m2.m20, m1.m21 + m2.m21,
        m1.m22 + m2.m22);
}
...
template<class T>
void Matrix3<T>::mul(const Matrix3 &m1) { mul(*this, m1); }
...
void Matrix3<T>::mulTransposeRight(const Matrix3 &m1, const Matrix3 &m2) {
    // alias-safe way.
    set(m1.m00 * m2.m00 + m1.m01 * m2.m01 + m1.m02 * m2.m02,
...
void Matrix3<T>::mulTransposeLeft(const Matrix3 &m1, const Matrix3 &m2) {
    // alias-safe way.
    set(m1.m00 * m2.m00 + m1.m10 * m2.m10 + m1.m20 * m2.m20,
...
bool Matrix3<T>::equals(const Matrix3 &m1) const {
    return m00 == m1.m00 && m01 == m1.m01 && m02 == m1.m02 && m10 == m1.m10 &&
           m11 == m1.m11 && m12 == m1.m12 && m20 == m1.m20 && m21 == m1.m21 &&
           m22 == m1.m22;
}
...
void Matrix3<T>::transform(const Tuple3<T> &t, Tuple3<T> *result) const {
    // alias-safe
    result->set(m00 * t.x + m01 * t.y + m02 * t.z,
...
template<class T>
VM_STRING_STD::string Matrix3<T>::toString() const { VM_TOSTRING }
...
VM_IOSTREAM_STD::ostream &operator<<(VM_IOSTREAM_STD::ostream &o,
    ...
    return o << "[ [" << t1.m00 << "," << t1.m01 << "," << t1.m02 << "]"
             << VM_IOSTREAM_STD::endl
             << "  [" << t1.m10 << "," << t1.m11 << "," << t1.m12 << "]"
             << VM_IOSTREAM_STD::endl

...
template<class T>
Matrix4<T>::Matrix4(T n00, T n01, T n02, T n03, T n10, T n11, T n12, T n13,
                    T n20, T n21, T n22, T n23, T n30, T n31, T n32, T n33)
        : m00(n00), m01(n01), m02(n02), m03(n03), m10(n10), m11(n11), m12(n12),
          m13(n13), m20(n20), m21(n21), m22(n22), m23(n23), m30(n30), m31(n31),
          m32(n32), m33(n33) {}

template<class T>
Matrix4<T>::Matrix4(const T v[])
        : m00(v[0]), m01(v[1]), m02(v[2]), m03(v[3]), m10(v[4]), m11(v[5]),
          m12(v[6]), m13(v[7]), m20(v[8]), m21(v[9]), m22(v[10]), m23(v[11]),
          m30(v[12]), m31(v[13]), m32(v[14]), m33(v[15]) {}

#ifdef VM_INCLUDE_CONVERSION_FROM_2DARRAY
template <class T>
Matrix4<T>::Matrix4(const T v[][4])
    : m00(v[0][0]), m01(v[0][1]), m02(v[0][2]), m03(v[0][3]), m10(v[1][0]),
      m11(v[1][1]), m12(v[1][2]), m13(v[1][3]), m20(v[2][0]), m21(v[2][1]),
      m22(v[2][2]), m23(v[2][3]), m30(v[3][0]), m31(v[3][1]), m32(v[3][2]),
      m33(v[3][3]) {}
#endif
...
template<class T>
void Matrix4<T>::set(T n00, T n01, T n02, T n03, T n10, T n11, T n12, T n13,
                     T n20, T n21, T n22, T n23, T n30, T n31, T n32, T n33) {
    m00 = n00;
    m01 = n01;
    m02 = n02;
...
#ifdef VM_INCLUDE_CONVERSION_FROM_2DARRAY
template <class T> void Matrix4<T>::set(const T m[][4]) {
  assert(m != 0);
  m00 = v[0][0];
  m01 = v[0][1];
  m02 = v[0][2];
...
template<class T>
T Matrix4<T>::getScale() const { return SVD(0, 0); }
...
template<class T>
void Matrix4<T>::sub(T scalar) { add(-scalar); }
...
void Matrix4<T>::invert() {
    ...
    set(m11 * (m22 * m33 - m23 * m32) + m12 * (m23 * m31 - m21 * m33) +
        m13 * (m21 * m32 - m22 * m31),
...
void Matrix4<T>::mulTransposeLeft(const Matrix4 &m1, const Matrix4 &m2) {
    // alias-safe way.
    set(m1.m00 * m2.m00 + m1.m10 * m2.m10 + m1.m20 * m2.m20 + m1.m30 * m2.m30,
...
bool Matrix4<T>::epsilonEquals(const Matrix4<T> &m1, T epsilon) const {
    return abs(m00 - m1.m00) <= epsilon && abs(m01 - m1.m01) <= epsilon &&
...
void Matrix4<T>::transform(const Tuple4<T> &vec, Tuple4<T> *vecOut) const {
    // alias-safe
    assert(vecOut != 0);
    vecOut->set(m00 * vec.x + m01 * vec.y + m02 * vec.z + m03 * vec.w,
...
void Matrix4<T>::transform(const Point3<T> &point, Point3<T> *pointOut) const {
    assert(pointOut != 0);
    pointOut->set(m00 * point.x + m01 * point.y + m02 * point.z + m03,
                  m10 * point.x + m11 * point.y + m12 * point.z + m13,
                  m20 * point.x + m21 * point.y + m22 * point.z + m23);
}

template<class T>
void Matrix4<T>::transform(const Vector3<T> &normal,
                           Vector3<T> *normalOut) const {
    normalOut->set(m00 * normal.x + m01 * normal.y + m02 * normal.z,
...
T Matrix4<T>::SVD(Matrix3<T> *rot3, Matrix4 *rot4) const {
    ...
    T s = VmUtil<T>::sqrt((m00 * m00 + m10 * m10 + m20 * m20 + m01 * m01 +
                           m11 * m11 + m21 * m21 + m02 * m02 + m12 * m12 +
                           m22 * m22) /
                          3.0);
...
template<class T>
VM_STRING_STD::string Matrix4<T>::toString() const { VM_TOSTRING }
...
VM_IOSTREAM_STD::ostream &operator<<(VM_IOSTREAM_STD::ostream &o,
                                     const VM_VECMATH_NS::Matrix4<T> &t1) {
    return o << "[ [" << t1.m00 << "," << t1.m01 << "," << t1.m02 << "," << t1.m03
             << "]" << VM_IOSTREAM_STD::endl
             << "  [" << t1.m10 << "," << t1.m11 << "," << t1.m12 << "," << t1.m13
             << "]" << VM_IOSTREAM_STD::endl
             << "  [" << t1.m20 << "," << t1.m21 << "," << t1.m22 << "," << t1.m23
             << "]" << VM_IOSTREAM_STD::endl

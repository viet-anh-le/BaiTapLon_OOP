...
public:
    ...
    T distanceLinf(const Point3 &p1) const {
        return VmUtil<T>::max(VmUtil<T>::abs(this->x - p1.x),
                              VmUtil<T>::abs(this->y - p1.y),
                              VmUtil<T>::abs(this->z - p1.z));
    }
...
template<class T>
inline VM_VECMATH_NS::Point3<T> operator*(T s,
                                          const VM_VECMATH_NS::Point3<T> &t1) {
...
template<class T>
inline VM_IOSTREAM_STD::ostream &
operator<<(VM_IOSTREAM_STD::ostream &o, const VM_VECMATH_NS::Point3<T> &t1) {

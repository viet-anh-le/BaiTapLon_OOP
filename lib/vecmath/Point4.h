...
public:
    ...
    T distanceLinf(const Point4 &p1) const {
        return VmUtil<T>::max(
                VmUtil<T>::abs(this->x - p1.x), VmUtil<T>::abs(this->y - p1.y),
                VmUtil<T>::abs(this->z - p1.z), VmUtil<T>::abs(this->w - p1.w));
    }
...
template<class T>
inline VM_VECMATH_NS::Point4<T> operator*(T s,
                                          const VM_VECMATH_NS::Point4<T> &t1) {
...
operator<<(VM_IOSTREAM_STD::ostream &o, const VM_VECMATH_NS::Point4<T> &t1) {

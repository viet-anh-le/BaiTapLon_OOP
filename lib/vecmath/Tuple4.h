 ...
 * @version specification 1.1, implementation $Revision: 1.5 $, $Date:
 * 1999/10/22 08:37:34 $
...
public:
    ...
    /**
     * Returns true if all of the data members of Tuple4 t1 are equal to the
    ...
    /**
     * Returns a string that contains the values of this Tuple4. The form is
     * (x,y,z,w).
     * @return the String representation
     */
#ifdef VM_INCLUDE_TOSTRING

    std::string toString() const { VM_TOSTRING }

#endif

    // copy constructor and operator = is made by complier

    bool operator==(const Tuple4 &t1) const { return equals(t1); }
    ...
    Tuple4 operator*(T s) const { return (Tuple4(*this)).operator*=(s); }
};

VM_END_NS

template<class T>
inline VM_VECMATH_NS::Tuple4<T> operator*(T s,
                                          const VM_VECMATH_NS::Tuple4<T> &t1) {
